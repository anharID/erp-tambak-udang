        ...
        <div class="w-full mx-auto px-4 sm:px-6 lg:px-8 text-gray-900 dark:text-gray-100 overflow-hidden">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                <div class="w- full p-6 overflow-hidden">
                    ...
                    <a href="{{ route('kelola_barang.create') }}" class="px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-blue-600 border border-transparent rounded-lg active:bg-blue-600 hover:bg-blue-700 focus:outline-none focus:shadow-outline-blue">
                    ...
                    <div class="w-full mt-4">
                        <table class="w-full table-auto mt-4 datatable">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                                @foreach($kelolajenisbarang as $row)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $row->jenisbarang }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap flex">
                                        <a href="{{ route('kelola_barang.edit', $row->id) }}" class="text-yellow-600 mr-4"><i class="fa-solid fa-pen-to-square"></i></a>
                                        <form action="{{ route('kelola_barang.destroy', $row->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
